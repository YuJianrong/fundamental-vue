...
@Doc.event('click', 'Sent when item is clicked')
@Doc.defaultSlot('Side Nav Items displayed by the list.')
export class SideNavItem extends mixins(IconMixin) implements ItemContainer {
    // TSX Support
    public $tsxProps!: Readonly<Props>;

    // Props
    @Doc.prop('whether selected', { type: Boolean, default: false })
    public isSelected!: boolean;

    @Doc.prop('whether is subitem', { type: Boolean, default: false })
    public isSubitem!: boolean;

    @Doc.prop('unique item identification', { type: String, required: true })
    public itemId!: string;

    @Doc.prop('submenu title', { type: String, default: null })
    public submenuTitle!: string | null;

    @Doc.prop('accessory icon (non-standard)', { type: String, default: null })
    public accessoryIcon!: string | null;

    @Doc.prop('accessory icon color(non-standard)', { type: String, default: 'accent-6' })
    public color!: Color | null;

    @Doc.prop('tooltip text(non-standard)', { type: String })
